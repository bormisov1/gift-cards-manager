  ...
  <div :class="['vue-tel-input', styleClasses, { disabled: disabled }]"
    ...
    >
      <span class="vti__selection"
        ...
        ></span>
        <span v-if="dropdownOptions.showDialCodeInSelection"
...
export default {
  ...
  computed: {
    ...
    phoneObject() {
      ...
      let valid = result.isValid && result.isValid();
      let formatted = this.phone;

      if (valid) {
        formatted = result.format && result.format(this.parsedMode.toUpperCase());
      }
  ...
  mounted() {
    ...
    this.initializeCountry()
      .then(() => {
        if (!this.phone
          && this.inputOptions && this.inputOptions.showDialCode
  ...
  methods: {
    ...
    choose(country) {
      ...
      if (this.inputOptions && this.inputOptions.showDialCode && parsedCountry) {
    ...
    toggleDropdown(e) {
      if (this.disabled || this.dropdownOptions.disabled || (e && e.path && e.path[0] && e.path[0].type === 'text')) {
        return;
      }
      this.searchQuery = '';
      this.open = !this.open;
